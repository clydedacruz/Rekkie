...
var MapsLib = {
  ...
  addrMarkerImage:    'assets/images/blue-pushpin.png',
  ...
  initialize: function(m,s,r) {
    ...
    map = new google.maps.Map($(MapsLib.mapDom)[0],myOptions);
    ...
    //reset filters
    $(MapsLib.searchDom).val(MapsLib.convertToPlainString($.address.parameter('address')));
    var loadRadius = MapsLib.convertToPlainString($.address.parameter('radius'));
    if (loadRadius != "") $(MapsLib.raduisDom).val(loadRadius);
    else $(MapsLib.raduisDom).val(MapsLib.searchRadius);
  ...
  doSearch: function(location) {
    MapsLib.clearSearch();
    var address = $(MapsLib.searchDom).val();
    MapsLib.searchRadius = $(MapsLib.raduisDom).val();
  ...
  addrFromLatLng: function(latLngPoint) {
    geocoder.geocode({'latLng': latLngPoint}, function(results, status) {
      if (status == google.maps.GeocoderStatus.OK) {
        if (results[1]) {
          $(MapsLib.searchDom).val(results[1].formatted_address);
          $('.hint').focus();
          MapsLib.doSearch();
        }
      } else {
  ...
  getlatlong:function(){
    return MapsLib.currentPinpoint;
  },
  
  getRaduis:function(){
      return MapsLib.searchRadius;
    
  },
  
  setSearchParam:function(value,raduis){
    $(MapsLib.searchDom).val('Verna Goa');
   $(MapsLib.raduisDom).val(1610);
  }
